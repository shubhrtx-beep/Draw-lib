...
class MotionRegistry:
    ...
    def compute_shape_state(self, shape: object, now: float, parse_color: Callable[[Any], Any], x=0.0, y=0.0, w=0.0, h=0.0, canvas=None) -> dict[str, Any]:
        ...
        for record in getattr(shape, "motion", []) or []:
    ...
    def _apply_record(
        ...
        if center_ref is not None and record.motion_type in ("rotate", "transform"):
    ...
    def _apply_builtin(
        ...
        if motion_type in {"move", "path", "position"}:
