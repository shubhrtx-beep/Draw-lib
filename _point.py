...
def _catmull_rom_point(
    ...
    """Evaluate a Cardinal / Catmull-Rom spline at parameter t in [0, 1]."""
    t2 = t * t
    t3 = t2 * t
    s = tension

    # Hermite basis functions with tangent scale s:
    # Tangent at p1: m1 = s * (p2 - p0)
    # Tangent at p2: m2 = s * (p3 - p1)
    # P(t) = (2*t^3 - 3*t^2 + 1)*p1 + (t^3 - 2*t^2 + t)*m1 + (-2*t^3 + 3*t^2)*p2 + (t^3 - t^2)*m2
    ...
    x = h00 * p1.x() + h10 * m1_x + h01 * p2.x() + h11 * m2_x
    y = h00 * p1.y() + h10 * m1_y + h01 * p2.y() + h11 * m2_y
    return QPointF(x, y)
...
def _build_curve_path(
    ...
    for seg in range(n - 1):
        p1 = pts[seg]
        p2 = pts[seg + 1]
        p0 = pts[seg - 1] if seg > 0 else QPointF(2.0 * p1.x() - p2.x(), 2.0 * p1.y() - p2.y())
        p3 = pts[seg + 2] if seg + 2 < n else QPointF(2.0 * p2.x() - p1.x(), 2.0 * p2.y() - p1.y())
...
class _PointCanvas(QWidget):
    ...
    def _draw_path(
        ...
        parent = self.parent()
        shared_canvas = getattr(parent, "_draw_canvas", None) if parent is not None else None
        if shared_canvas is not None and not (p.ip and p.ip.startswith("scroller_")):
            dx -= getattr(shared_canvas, "_scroll_x", 0.0)
            dy -= getattr(shared_canvas, "_scroll_y", 0.0)
        if dx or dy:
