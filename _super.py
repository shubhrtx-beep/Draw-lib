...
from Draw._shapes import (
    ShapeDef, _regular_polygon_points, _shape_preferred_geometry,
...
class _DrawOpenGLCanvas(QOpenGLWidget):
    ...
    def _do_paint_gl(self) -> None:
        ...
        for s in sorted_shapes:
            ...
            if getattr(s, "_is_dragged", False) and not is_scroller:
            ...
            # Update spatial index for instant mouse hit testing
            self.spatial_grid.update(id(s), (float(anim_x), float(anim_y), float(anim_sw), float(anim_sh)))

            # Direct GPU Batching for standard regular polygons (convex, solid color, no complex curves/cutouts)
            is_simple_polygon = (
            ...
            batched = False
            if is_simple_polygon and self._gl_initialized and not self._gpu_failed:
                ...
                rot = getattr(s, "rotation", 0.0) or 0.0
                if rot != 0.0:
                ...
                batched = self.batcher.add_polygon(pts, color_rgba)
                if batched:
                    batched_shape_set.add(id(s))
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:  # noqa: N802
        ...
        txt = event.text()
        key_name = txt if (txt and txt.isprintable() and len(txt) == 1) else _qt_key_to_name(event.key())
