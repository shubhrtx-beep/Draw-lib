...
class DebugManager:
    ...
    def safe_stop(self, reason: str = "Emergency Stop"):
        ...
        # Close all Draw windows cleanly
        try:
    ...
    def _check_infinite_loop(self):
        ...
        If the main thread is stuck at the exact same line of Python code for
        multiple watchdog ticks without updating frames, AND main thread CPU is high,
        flag infinite loop and request cooperative shutdown or (opt-in) force interrupt.
        """
        ...
        # Ignore internal Qt event loop blocking wait frames (e.g. app.exec, _window.py:run)
        if (
            "PySide6" in loc[0]
            or "exec" in loc[2]
            or "run" in loc[2]
            ...
            self._main_frame_stuck_count = 0
            self._last_main_frame_location = loc
            return
        ...
        if self._main_frame_stuck_count >= self._settings.infinite_loop_stuck_threshold:
            fn_name = os.path.basename(loc[0])
            msg = (
                f"Possible Infinite Loop detected at {fn_name}:{loc[1]} in '{loc[2]}'. "
                f"Main thread stuck for >{self._settings.infinite_loop_stuck_seconds:.1f} seconds."
            )
            self._logger.log(LogLevel.CRITICAL, msg, "crashes")

            if self._settings.auto_emergency_stop_enabled:
                self.safe_stop("Infinite Loop Detected on Main Thread")
                if self._settings.force_thread_interrupt:
                    try:
                        ctypes.pythonapi.PyThreadState_SetAsyncExc(
                            ctypes.c_long(main_id),
                            ctypes.py_object(SystemExit)
                        )
                    except Exception as exc:
                        self._logger.log(LogLevel.ERROR, f"Failed to interrupt main thread: {exc}", "crashes")
