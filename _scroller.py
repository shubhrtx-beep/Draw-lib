...
class _ScrollerRegistry:
    ...
    def scroll_to(
        ...
        *,
        ...
        bound_max_x = cfg.max_x if cfg and cfg.max_x is not None else (max_scroll_x if max_scroll_x > 0 else None)
        bound_max_y = cfg.max_y if cfg and cfg.max_y is not None else (max_scroll_y if max_scroll_y > 0 else None)

        target_x = max(0.0, target_x)
        if bound_max_x is not None:
            target_x = min(float(bound_max_x), target_x)

        target_y = max(0.0, target_y)
        if bound_max_y is not None:
            target_y = min(float(bound_max_y), target_y)
    ...
    def _register_connectors(self, cfg: ScrollerConfig) -> None:
        ...
        def _make_wheel_work(delta_y: float, delta_x: float = 0.0):
            ...
            def _work(record):
                ...
                max_scroll_x, max_scroll_y = canvas._get_max_scroll_range() if hasattr(canvas, "_get_max_scroll_range") else (0.0, 0.0)
                bound_max_y = cfg.max_y if cfg.max_y is not None else (max_scroll_y if max_scroll_y > 0 else None)
                bound_max_x = cfg.max_x if cfg.max_x is not None else (max_scroll_x if max_scroll_x > 0 else None)

                if is_vert and delta_y != 0.0:
                    new_y = max(0.0, canvas._scroll_y + delta_y)
                    if bound_max_y is not None:
                        new_y = min(float(bound_max_y), new_y)
                    if snap:
                ...
                if is_horiz and (delta_x != 0.0 or not is_vert):
                    ...
                    new_x = max(0.0, canvas._scroll_x + dx)
                    if bound_max_x is not None:
                        new_x = min(float(bound_max_x), new_x)
                    if snap:
