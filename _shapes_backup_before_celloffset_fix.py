...
class _DrawCanvas(QWidget):
    ...
    def _update_scroller_thumbs(self) -> None:
        """Reposition scroller thumbs to match current scroll offsets."""
        ...
        for cfg in self._scroller_configs:
            ...
            if cfg["direction"] == "vertical":
                ...
                thumb_h = float(cfg["thumb_h"])
                travel  = max(1.0, track_h - thumb_h)
                # One canvas-height of scroll = full travel
                scroll_range = max(1.0, float(ch))
                t = max(0.0, min(1.0, self._scroll_y / scroll_range))
                new_y = cfg["track_y"] + t * travel
                thumb.y = int(new_y)
                thumb.last_position = (float(cfg["track_x"]), float(new_y))
            else:
                track_w = float(cfg["track_w"])
                thumb_w = float(cfg["thumb_w"])
                travel  = max(1.0, track_w - thumb_w)
                scroll_range = max(1.0, float(cw))
                t = max(0.0, min(1.0, self._scroll_x / scroll_range))
        ...
        try:
            ...
            for item_tuple in draw_queue:
                ...
                if kind_str == "shape":
                    ...
                    if not (s.ip and s.ip.startswith("scroller_")):
                    ...
                    # the layout/overlap system computed as its "placed" position. This
                    # must happen here — after placement, before motion — so motion still
                    # animates on top of the correct base position and the shape doesn't
                    # snap back to its placed spot on the next repaint.
                    if getattr(s, "_is_dragged", False):
...
def _build_shape_path(
    ...
    elif motion_state and "vertices_count" in motion_state:
...
def _draw_one_shape(
    ...
    if eff_border_width > 0 and eff_border_style != "none":
        pen = QPen(eff_border_color, eff_border_width)
        ...
        **shape_props,                       # single-shape shorthand: Draw.shape(tag=W, ip=.., type=.., ...)
        ...
        if sub_edit is not None:
            ...
            if edit_ips:
                ...
                canvas._occupied_dirty = True
                canvas.update()
                return  # sub_edit only — no new shapes appended
